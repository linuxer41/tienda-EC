...
$(function($) {
	...
	$(".j-filter-reset").click(function() {
		...
		$(".j-menu-select .j-t-jiantou em").text("Todos");
		$(".j-filter-city span.text-all-span").css("color", "#555");
		$(".j-filter-city span.text-all-span").text("Por favor seleccione");
		$(".div-messages").removeClass("active");
		$(this).parents(".j-get-limit").attr("data-istrue", true)
	});
	...
	$(".j-get-i-more .j-select-btn").click(function() {
		if($(this).parents(".ect-select").hasClass("j-flowcoupon-select-disab")) {
			d_messages("Solo se puede seleccionar uno del mismo comerciante", 2);
		} else {
			is_select_all = true;
		...
		if(!$(this).parent(".div-num").hasClass("div-num-disabled")) {
			...
			if($(this).hasClass("num-plus")) {
				...
				if(num < max_num) {
					...
					d_messages("No puede exceder el número máximo", "2");
				}
				return false;
			}
		} else {
			d_messages("El producto no se puede aumentar ni disminuir.", "2");
		}
	});
	$(".div-num input").bind("change", function() {
		...
		if(num < min_num) {
			$(this).val(min_num);
			d_messages("No menos de la cantidad mínima", "2");
			return false;
		}
	});
		...
		if(!$(this).parent(".div-num").hasClass("div-num-disabled")) {
			if($(this).hasClass("num-less")) {
				...
				if(num > min_num) {
					...
					d_messages("No menos de la cantidad mínima");
				}
				return false;
			}
			if($(this).hasClass("num-plus")) {
				...
				if(num < max_num) {
					...
					d_messages("No puede exceder el número máximo");
				}
				return false;
			}
		} else {
			d_messages("El producto no se puede aumentar ni disminuir.");
		}
	});
	$(".div-num input").bind("change", function() {
		num = parseInt($(this).val());
